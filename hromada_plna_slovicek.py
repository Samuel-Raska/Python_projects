...
Program procvicuje slovicka, ktera jsou nahrana v jednotlivych souborech:
...
---------- ÚVOD ----------
-> Procvičte si dané slovíčko několikrát, což zvyšuje pravděpodobnost, že se slovíčko uloží do dlouhodobé paměti.
...
def ukonceni_sekvence(vstup: str):
    '''
    Funkce kontroluje vstupni parametry, zda-li chce uzivatel ukoncit sekvenci.
    Pokud je zadany ukoncovaci znak (Q, q, ENTER0, tak vrati True, jinak False.

    :param vstup: Vstupni data od uzivatele.
    :type: str
    :return: Vysledek porovnani vstupu s ukoncovacimi znaky.
    :rtype: Bool
    '''
    if vstup == 'Q':
        return True
...
def presun_slovicka(hromadka_5: str = HROMADKA_5,
    ...
    while True:
        ...
        if ukonceni_sekvence(pocet_k_presunu):
            break
...
Pro ukončení sekvence zadejte ENTER:\n''')
    ...
    while True:
        ...
        if ukonceni_sekvence(rozhodnuti):
...
def kontrola_slovicek_cz_to_eng(**kwargs: dict):
    ...
    Uzivateli se zobrazuje kolikatou frazi z hromadky uz procvicuje, zacina se od cisla 1.
    ...
    spatne_odpovedi = {}
    for key, value in kwargs.items():
        odpoved = input(f'{pocitadlo_poradi}.Jak se "{key}" řekne anglicky? -> ')
        if ukonceni_sekvence(odpoved):
            menu()
            break
...
def kontrola_slovicek_eng_to_cz(**kwargs: dict):
    ...
    spatne_odpovedi = {}
    for key, value in kwargs.items():
        odpoved = input(f'{pocitadlo_poradi}.Jak se "{value}" řekne česky? -> ')
        if ukonceni_sekvence(odpoved):
            menu()
            break
...
def procvicit_slovicka(hromadka_0: str = HROMADKA_0,
    ...
    Behem provadeni se nacitaji spravne a spatne odpovedi (int).
